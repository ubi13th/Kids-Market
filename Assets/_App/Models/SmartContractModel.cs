...
{
    [JsonProperty] public bool IsSurprise;
    [JsonProperty] public string Title;
    [JsonProperty] public float RewardAmount;
    [JsonProperty] public string StartDate; // "yyyy-MM-dd"
    [JsonProperty] public RepeatType RepeatMode = RepeatType.EveryDay;
    [JsonProperty] public List<DayOfWeek> RepeatDays = new();
    [JsonProperty] public string DueTime;   // "HH:mm"

    [JsonProperty] public bool RequirePhotoProof;
    [JsonProperty] public bool RequireParentalApproval;
    [JsonProperty] public bool RequireNotificationOnThisDevice;

    [JsonProperty] public string IconPath;
    [JsonProperty] public string Id;
    [JsonProperty] public string OriginalId;
    ...
    [JsonProperty] public string stateHistoryRaw;
    ...
    {
        try { DueTime = time.ToString(@"hh\:mm"); }
        catch (FormatException e) { Debug.LogError($"❌ Invalid DueTime: {e.Message}"); DueTime = "00:00"; }
    }

    public TimeSpan GetDueTime() => TimeSpan.TryParse(DueTime, out var time) ? time : TimeSpan.Zero;
    ...
    public void LoadStateHistory()
    {
        ...
        {
            Debug.LogWarning($"⚠️ stateHistoryRaw is null or empty for contract {Id}");
            return;
        }
            ...
            foreach (var entry in entries)
            {
                ...
                for (int i = 1; i < segments.Length; i++)
                {
                        ...
                        int.TryParse(sub[1], out var stateValue))
                    {
                        ...
                        var record = new StateRecord { QueueId = key, State = (SmartContractState)stateValue };
        ...
        foreach (var entry in flatEntries)
        {
            ...
            if (parts.Length == 2 &&
                DateTime.TryParseExact(parts[0], "yyyy-MM-dd", null, DateTimeStyles.None, out _) &&
                int.TryParse(parts[1], out int stateValue))
            {
                string key = parts[0];

                var record = new StateRecord { QueueId = null, State = (SmartContractState)stateValue };
    ...
    public void SetStateOnDate(DateTime date, SmartContractState state)
    {
        ...
        StateHistory[key].Add(new StateRecord { QueueId = null, State = state });
    ...
    public void SetStateOnDateWithQueue(DateTime date, SmartContractState state, int queueIndex)
    {
        ...
        StateHistory[key].Add(new StateRecord { QueueId = key, State = state });
    ...
    public string TryGetMatchingKey(DateTime date)
    {
        ...
        return StateHistory.Keys.FirstOrDefault(k => k == prefix || k.StartsWith(prefix + "#"));
    }
...
public class SmartContractModel
{
    ...
    [SerializeField, JsonProperty("IsCopy")]
    private bool isCopy;
    
    [JsonIgnore] // prevent double-serialization
    public bool IsCopy
    {
        get => isCopy;
        set => isCopy = value;
