...
public class ScrollMechanic : MonoBehaviour, IDropHandler, IDragHandler, IBeginDragHandler, IPointerExitHandler, IPointerEnterHandler {
    ...
    public float maxFontSize = 55f; //Maximun font size
    ...
    public void Initialize(List<string> dataToInit, bool isInfinite = false, int firstTarget = 0) {
        ...
        // Debug.Log($"[🧱 CHILD COUNT] {contentTarget.childCount} total children in {name}");
        // for (int i = 0; i < contentTarget.childCount; i++)
        // {
        //     var go = contentTarget.GetChild(i);
        //     Debug.Log($"[🔢 INDEX] {i} ➜ Name: {go.name}, PosY: {go.GetComponent<RectTransform>().anchoredPosition.y}");
        // }
    }
    ...
    // {
    //     int paddedIndex = currentCenter - padCount;
    //     paddedIndex = Mathf.Clamp(paddedIndex, 0, countTotal - 1);
    //     Debug.Log($"[📍 GetCurrentValue FIXED] currentCenter: {currentCenter}, PaddedIndex: {paddedIndex}, Returned Value: {paddedIndex}, Scroll: {name}");
    //     return paddedIndex;
    // }
    ...
    {
        // Log the inputs and calculated position
        float positionY = (value + 3) * (heightText * 2);
        //Debug.Log($"[🌀 ScrollToValue] Value: {value}, Calculated Y: {positionY}, heightText: {heightText}, Target name: {name}");

        // Apply the position
        contentTarget.anchoredPosition = new Vector2(0, positionY);
    }
    ...
    private void Update() {
        ...
        if (isInitialized) {
            ...
            //Debug.Log($"[📍 currentCenter calc] Y: {contentPos}, heightText*2: {heightText * 2}, Result: {currentCenter}, Scroll: {name}");
